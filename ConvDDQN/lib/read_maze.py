...
# get local 3*3 information centered at (x,y).
def quick_get_local_maze_information(x, y):
    global maze_cells
    random_location = random.choice(flag_list)
    around = np.zeros((3, 3, 2), dtype=int)

    for a,b in fires:
        if maze_cells[a][b][1] == 0:
            pass
        else:
            maze_cells[a][b][1] = maze_cells[a][b][1] - 1  # decrement the fire time
    ...
    for i in range(3):
        for j in range(3):
            ...
            if i == random_location // 3 and j == random_location % 3:
                ...
                fires.append((x - 1 + i, y - 1 + j)) # added
